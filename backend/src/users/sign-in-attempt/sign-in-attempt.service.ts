...
export class SignInAttemptService {
    private static lockoutPeriodReached = (attemptDate: Date):boolean => Date.now() - attemptDate!.getTime() > TEN_MINUTES
    constructor(
    ...
    async isLockedOut(user: UserEntity) {
        ...
        if (SignInAttemptService.lockoutPeriodReached(attemptedAt!))
            return false
    ...
    async updateSignInAttemptCountAfterWrongTry(user: UserEntity) {
        ...
        if (count < ALLOWED_TRIES)
            newCount = count+1
        else if (SignInAttemptService.lockoutPeriodReached(attemptedAt!))
            newCount = 1
        else
