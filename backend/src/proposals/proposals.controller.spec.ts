...
describe.only(`/api/v1/proposals`, () => {
    ...
    describe('GET /?network=networkName', () => {
        ...
        it(`should return ${HttpStatus.OK} if network query param is given`, () => {
            return request(app())
                .get(`${baseUrl}?network=localhost`)
                .expect(HttpStatus.OK)
        })

        it(`should return ${HttpStatus.BAD_REQUEST} if network query param is missing`, () => {
            return request(app())
                .get(baseUrl)
